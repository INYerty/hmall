...
//    private final DiscoveryClient discoveryClient;
    ...
    public List<CartVO> queryMyCarts() {
        // 1.查询我的购物车列表
        List<Cart> carts = lambdaQuery().eq(Cart::getUserId, UserContext.getUser() /*TODO：UserContext.getUser() 已完成*/).list();
...
/*        List<ItemDTO> items = itemService.queryItemByIds(itemIds);
        ...
        //手写负载均衡，从服务列表中获取一个实例对象，拿到其ip:port
        ServiceInstance serviceInstance = instances.get(RandomUtil.randomInt(instances.size()));
        URI uri = serviceInstance.getUri();
        ResponseEntity<List<ItemDTO>> response =
                restTemplate.exchange(uri + "/items?ids={ids}",
                        HttpMethod.GET,
                        null,
    ...
    @Override
    public void removeByItemIdsWAY2(Set<Long> itemIds, Long userId) {
        remove(new QueryWrapper<Cart>().lambda()
